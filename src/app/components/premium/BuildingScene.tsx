...
export default function BuildingScene({
  ...
  useEffect(() => {
    ...
    const mat = {
      ...
      glass: new THREE.MeshPhysicalMaterial({ color: "#79aeb1", roughness: 0.03, transmission: 0.38, transparent: true, opacity: 0.5, metalness: 0.06, side: THREE.DoubleSide }),
      railGlass: new THREE.MeshPhysicalMaterial({ color: "#b9d4d2", roughness: 0.02, transmission: 0.48, transparent: true, opacity: 0.34, side: THREE.DoubleSide }),
      ...
      water: new THREE.MeshPhysicalMaterial({ color: "#4daec0", roughness: 0.04, transmission: 0.15, transparent: true, opacity: 0.82 }),
    };
    ...
    if (ctx) {
      ...
      ctx.font = "700 164px Arial";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText("SBRE", 512, 108);
      ctx.font = "600 54px Arial";
      ctx.fillText("INGÉNIERIE", 512, 204);
    }
    ...
    const balconySign = new THREE.Mesh(new THREE.PlaneGeometry(3.35, 0.86), labelMat);
    ...
    const keyDown = (e: KeyboardEvent) => {
      if (e.key === "ArrowLeft") { manualOffset -= 0.18; e.preventDefault(); start(); }
      else if (e.key === "ArrowRight") { manualOffset += 0.18; e.preventDefault(); start(); }
    };
