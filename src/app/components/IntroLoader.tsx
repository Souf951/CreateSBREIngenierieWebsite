...
const INTRO_CUES: ReadonlyArray<{
  at: number;
  action: "logo" | "tagline" | "move" | "hide" | "complete";
}> = [
  { at: 2500, action: "logo" },
  { at: 3500, action: "tagline" },
  { at: 4500, action: "move" },
  { at: 4900, action: "hide" },
  { at: 5200, action: "complete" },
];
...
export default function IntroLoader({ onComplete, onReveal }: IntroLoaderProps) {
  ...
  const skip = useCallback(() => {
    ...
    window.setTimeout(() => onCompleteRef.current(), 850);
  }, []);
  ...
  const villaOpacity =
    phase === "logo" || phase === "tagline" || phase === "move" ? 0.12 : 1;
  const villaStroke =
    phase === "logo" || phase === "tagline" || phase === "move" ? 0.55 : 0.4;
  const logoPhase = phase === "logo" || phase === "tagline" || phase === "move";

  const drawProps = (
    group: keyof typeof DRAW_START,
    duration = 1.2,
    delay = 0,
  ) => ({
    pathLength: 1,
    ...
    transition: {
      ...
      delay: DRAW_START[group] + delay * 0.5,
    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Introduction SBRE Ingénierie"
        >
          <button
            onClick={skip}
            onKeyDown={(event) => {
          ...
          >
            ...
            <motion.line
              ...
              y2={95}
              stroke={STROKE}
              strokeWidth={villaStroke}
              {...drawProps("foundation", 1)}
            />
